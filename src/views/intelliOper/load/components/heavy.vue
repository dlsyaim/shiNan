...
<script>
import { getHeavyModuleDayData } from '@/api/api'
import { regionMonthStaticsForZZ } from '@/api/api'
...
export default {
  ...
  data(){
    return {
      ...
      // 当日显示信息
      displayTodayList: [
        {code: 'zz',text: '重载',value: '0', unit: '台'},
        {code: 'zzl',text: '重载率',value: '0 %', unit: ''},
      ],
      // 连续30天显示信息
      displayDaysList: [
      ...
      displayMonthList: [
        {code: 'zz',text: '重载',value: '0', unit: '台'},
        {code: 'lxst',text: '连续3天',value: '1', unit: '台'},
        {code: 'ljqt',text: '累计7天',value: '2', unit: '台'},
      ],
      // 当日与本月选项
      checkedValue: '当日',
      checkOptions: [
      ...
      defectMonthOption: {
        xAxis: [{
            ...
            // data: ["一般重载", "严重重载", "连续3天重载", "累计7天重载"]
            data: ["重载数", "连续3天重载", "累计7天重载"]
        }],
        yAxis: [{
          type: "value",
          axisLine: {
  ...
  mounted(){
    getHeavyModuleDayData().then(res => {
      if(res.data.length){
        ...
        // regionMonthStaticsForZZ.getMonthStaticsData(currentMonth).then(res => {
           regionMonthStaticsForZZ.getMonthStaticsData('',currentMonth).then(res => {
  ...
  methods:{
    ...
    datePickerHandleChange() {
      ...
      getHeavyModuleDayData(currentDate).then(res => {
        this.setDisplayList(res.data, 'Today');
        this.setDisplayList(res.data, 'Days');
        this.renderModule(res.data, 'Today');
        this.renderModule(res.data, 'Days');
      ...
      if(type === 'Today' || type === 'Days'){
        ...
        if(type === 'Days'){
          for(let j=0;j<data.length;j++){
            ...
            if(j === 0){
              for(let i=0;i<data.length;i++){
                seriesData[j].push(Number(data[i].LX3ZZS));
              }
            }else if(j === 1){
              for(let i=0;i<data.length;i++){
                seriesData[j].push(Number(data[i].LJ7ZZS));
              }
            }
          }
        }else if(type === 'Today'){
          ...
          for(let j=0;j<data.length;j++){
            ...
            if(j === 0){
              for(let i=0;i<data.length;i++){
                seriesData[j].push(Number(data[i].ZZS));
              }
            }else if(j === 1){
              for(let i=0;i<data.length;i++){
                seriesData[j].push(Number(data[i].ZZL));
              }
            }
          }
        }
        ...
        if(data !== null && data.length){
          seriesData.push(Number(data[0].ZZS))
          seriesData.push(Number(data[0].LX3ZZS))
          seriesData.push(Number(data[0].LJ7ZZS))
    ...
    setDisplayList(data, type){
      if(type === 'Today'){
        ...
        let zzl = 0;
        for(let i=0;i<data.length;i++){
          zzValue += Number(data[i].ZZS);
          zzl += Number(data[i].ZZL);
        }
        this.displayTodayList[0].value = zzValue;
        this.displayTodayList[1].value = zzl + ' %';
      }else if(type === 'Days'){
        let lxstValue = 0;
        let ljqtValue = 0;
        for(let i=0;i<data.length;i++){
          lxstValue += Number(data[i].LX3ZZS);
          ljqtValue += Number(data[i].LJ7ZZS);
        }
        this.displayDaysList[0].value = lxstValue;
        this.displayDaysList[1].value = ljqtValue;
      }else if(type === 'Month'){
        if(data !== null && data.length){
          // this.displayMonthList[0].value = data[0].FSCS;
          // this.displayMonthList[1].value = data[0].LXTS;
          // this.displayMonthList[2].value = data[0].LJTS;
          this.displayMonthList[0].value = data[0].ZZS;
          this.displayMonthList[1].value = data[0].LX3ZZS;
          this.displayMonthList[2].value = data[0].LJ7ZZS;
        }
      }
