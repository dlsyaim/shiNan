...
<script>
import { getLowVoltageModuleTodayData } from '@/api/api'
import { regionMonthStaticsForDDY } from '@/api/api'
import { getLowVoltageModuleRateData } from '@/api/api'
...
export default {
  ...
  data(){
    return {
      ...
      // 当日显示信息
      displayTodayList: [
        {code: 'ddy',text: '低电压',value: '0', unit: '台'},
        {code: 'yzddy',text: '严重低电压',value: '0', unit: '台'},
        {code: 'ddyl',text: '低电压率',value: '0 %', unit: ''},
      ],
      // 连续30天显示信息
      displayDaysList: [
      ...
      // 当月显示信息
      displayMonthList: [
        {code: 'yzgz',text: '低电压',value: '0', unit: '台'},
        {code: 'lxst',text: '连续3天',value: '0', unit: '台'},
        {code: 'ljqt',text: '累计7天',value: '0', unit: '台'},
        {code: 'ddyl',text: '低电压率',value: '0 %', unit: ''},
      ],
  ...
  mounted(){
    ...
    getLowVoltageModuleTodayData().then(res => {
      if(res.data.length){
    ...
    getLowVoltageModuleRateData().then(res => {
  ...
  methods:{
    ...
    datePickerHandleChange() {
      ...
      getLowVoltageModuleTodayData(currentDate).then(res => {
        this.setDisplayList(res.data, 'Today');
        this.setDisplayList(res.data, 'Days');
        this.renderModule(res.data, 'Today');
        this.renderModule(res.data, 'Days');
    ...
    renderModule(data, type){
      ...
      if(type === 'Today' || type === 'Days'){
        ...
        if(type === 'Days'){
          ...
          for(let j=0;j<data.length;j++){
            ...
            if(j === 0){
              ...
              for(let i=0;i<data.length;i++){
                seriesData[j].push(Number(data[i].DDYL));
              }
            }
          }
        }
        ...
        if(data !== null && data.length){
          ...
          seriesData.push(Number(data[0].DDYS))
          seriesData.push(Number(data[0].LX3DDY))
          seriesData.push(Number(data[0].LJ7DDY))
          seriesData.push(Number(data[0].DDYL))
        }
    ...
    setDisplayList(data, type){
      if(type === 'Today'){
        ...
        let ddyl = 0;
        for(let i=0;i<data.length;i++){
          ddyValue += Number(data[i].DDYS);
          yzddyValue += Number(data[i].YZDDYS);
          ddyl += Number(data[i].DDYL);
        }
