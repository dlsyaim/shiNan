...
<script>
import { getWaringData ,getWireData } from '@/api/api'
...
export default {
  ...
  data(){
    return {
      ...
      displayMonthList: [],
      // 当前显示信息
       currentDisplayList: [ {code: 'pzfzl',text: '过电压',value: '', unit: '台'},{code: 'pzfzl',text: '过电压较严重',value: '', unit: '台'},{code: 'pzfzl',text: '过电压率',value: '-- %', unit: ''},],
      // 当日与本月选项
      checkedValue: '当日',
      checkOptions: [
  ...
  mounted(){
   ...
   getWaringData().then(res => {
       ...
       res.data.forEach(item => {
         let PJFZL = item.GDYS*1;
         sum = sum*1 + PJFZL*1
         xnewData.push(item.RTU_DEPT);
         seriesData0.push(item.GDYS);
         seriesData1.push(item.YZGDYS);
         if(item.GDYL == null){
             seriesData2.push(item.GDYL = 0);    
         } else {
           seriesData2.push(item.GDYL);    
         } 
        })
        console.log('xnewData',xnewData)
      ...
      if(!this.todayData.length){
        ...
        getLightTodayData(queryParam).then(res => {
          if (res.code === 200) {
            ...
            this.renderEchart("OtherPieTwo", this.defectOtherOption);
          } else {
            this.renderEchart("OverVoltageCharts", this.defectTodayOption, this.defaultTodayData);
            this.renderEchart("OtherPieTwo", this.defectOtherOption, this.defaultTodayData);
            this.$message.error(res.message);
          }
        }).catch((err) => {
          this.$message.error(err);
